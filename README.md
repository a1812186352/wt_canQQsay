...
基于 QQ 聊天场景的多 Agent 演示项目。内置**小Q 智能管家**，按联系人授权后自动记录对话内容，消息达到阈值后生成内容总结；同时支持悬浮球一键触发智能回复、话术优化、精华提炼。可接入 DeepSeek 等 LLM API 驱动智能回复。

## 功能概览

| 功能 | 说明 |
|------|------|
| 小Q 智能管家 | 常驻好友栏首位，按联系人分别授权监测，消息累计达阈值自动生成对话总结 |
| 按联系人权限 | 每个好友可独立设置：记录聊天消息、自动内容总结、上下文长度阈值、未回复消息提醒及提醒时长 |
| 未回复消息提醒 | 用户自行开启，设置提醒时长；已读对方消息超时未回复时小Q主动推送提醒 |
| 聊天 Agent | 智能回复建议、话术优化、聊天精华提炼（悬浮球一键触发） |
| 画像系统 | 自然语言展示联系人偏好、沟通风格、常聊话题，注入回复 prompt |
| 行为日志 | 记录消息/AI调用事件，驱动画像更新 |
...
1. **确认已进入 docs 目录** — 必须在 `docs/` 下启动，不是在项目根目录
...
- **改推送规则** → 编辑 `config/agent-rules.json`
- **改画像模板** → 编辑 `profiles/*.json`（运行时 fetch 加载，修改后刷新即生效）
- **改小Q权限默认值** → 编辑 `js/config.js` 中 `XiaoQMemory.getPerms` 的 fallback
- **改样式** → 编辑 `styles.css`
...
全部数据本地存储，不上传服务器。仅 LLM 调用走外部 API。不同用户访问 GitHub Pages 时数据完全隔离，各自存储在本地浏览器中。
